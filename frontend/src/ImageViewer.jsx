    ...
    for (const file of filesToUpload) {
      ...
      try {
        ...
        setContainers((prev) => {
          ...
          applyRGBFromFiles(targetContainerName, updatedFiles);
    ...
    <div style={styles.appContainer}>
      ...
      <div style={styles.mainContent}>
          ...
          <div style={styles.toolGroup}>
            ...
            <div style={styles.btnRow}>
              <button
                style={{
                  ...styles.iconBtn,
                  opacity: activeFilesPool.length > 0 && !isSwipeMode ? 1 : 0.4,
                }}
                onClick={openHistogramModal}
                disabled={activeFilesPool.length === 0 || isSwipeMode}
              >
                📊 Histogram
              </button>
              <button
              ...
              <button
                style={{
                  ...styles.iconBtn,
                  background: isProfileMode ? "#0d9488" : "#1e222d",
                  opacity: activeFilesPool.length > 0 && !isSwipeMode ? 1 : 0.4,
                }}
                onClick={() => {
                  if (activeFilesPool.length === 0 || isSwipeMode) return;
                  if (isProfileMode) {
                    setIsProfileMode(false);
                    showToast("Profile mode canceled", "success");
                  } else {
                    openProfilePlotModal();
                  }
                }}
                disabled={activeFilesPool.length === 0 || isSwipeMode}
              >
        ...
        <div
          ...
          style={{ ...styles.viewport, cursor: isProfileMode ? "crosshair" : isDragging ? "grabbing" : "grab" }}
        >
          {isSwipeMode ? (
            ...
            <img ref={imgRef} src={displayedImageUrl} alt="Viewport" draggable={false} style={getImageStyle} />
          ) : (
            <div style={styles.placeholder}>Select a container to start</div>
          )}
          {isImageLoading && !isSwipeMode && (
            <div style={styles.loadingBadge}>
              <div style={styles.spinnerSmall} /><span>Rendering...</span>
            </div>
          )}
          {!isSwipeMode && displayedImageUrl && showMinimap && (
            <div
              ...
              onClick={(e) => {
                if (!imgRef.current || !containerRef.current) return;
                const miniEl = e.currentTarget.getBoundingClientRect();
                const clickX = e.clientX - miniEl.left;
                const clickY = e.clientY - miniEl.top;
                const clickPctX = clickX / miniEl.width;
                const clickPctY = clickY / miniEl.height;
                const imgRect = imgRef.current.getBoundingClientRect();
                const contRect = containerRef.current.getBoundingClientRect();
                const clickDispX = clickPctX * imgRect.width;
                const clickDispY = clickPctY * imgRect.height;
                const desiredImgLeft = contRect.left + (contRect.width / 2) - clickDispX;
                const desiredImgTop = contRect.top + (contRect.height / 2) - clickDispY;
                const deltaX = desiredImgLeft - imgRect.left;
                const deltaY = desiredImgTop - imgRect.top;
                setPosition((prev) => ({ x: prev.x + deltaX, y: prev.y + deltaY }));
              }}
              style={{ ...styles.minimapContainer, right: (showHistogram || showScatterPlot || showProfileModal) ? "378px" : "18px" }}
            >
